...
export const adminLogin = (email, password) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post(`${API_ENDPOINT}/admin/login`, { email, password, isAdmin: true }, config);
...
export const adminViewProfile = (admin) => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.get(`${API_ENDPOINT}/admin/view`, admin, config);
...
export const adminUpdateProfile = (admin) => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.put(`${API_ENDPOINT}/admin/edit`, admin, config);
